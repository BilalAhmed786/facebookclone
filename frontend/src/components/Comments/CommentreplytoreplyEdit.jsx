...
const CommentreplytoreplyEdit = ({
  ...
  // Close editor on Escape key press
  useEffect(() => {
    ...
    const getComment = async () => {
      try {
        ...
        const response = await axios.get(
          `${backendurl}/api/comments/replytoreplyedit/${commenteditid}`,
        ...
        if (isMounted) {
          toast.error(
            error.response?.data?.msg || 'Failed to fetch comment reply'
          );
          seteditCommentvisible(false);
        }
      } finally {
  ...
  const handleComment = async (e) => {
    ...
    try {
      ...
      const result = await axios.put(
        `${backendurl}/api/comments/replytoreplyupdate/${commenteditid}/${replyid}/`,
        { comment },
        { withCredentials: true }
      );
