...
const Commentreplyedit = ({
  ...
  useEffect(() => {
    ...
    const getCommentReply = async () => {
      try {
        ...
        const response = await axios.post(
          `${backendurl}/api/comments/childcommentedit/${commenteditid}`,
          { commentreplyid },
        ...
        if (isMounted && response?.data) {
          const text =
            typeof response.data === 'string'
              ? response.data
              : response.data.text || response.data.comment || '';
          setComment(text);
        }
      } catch (error) {
        if (isMounted) {
          toast.error(
            error.response?.data?.msg || 'Failed to load reply text'
          );
          seteditCommentvisible(false);
        }
      } finally {
    ...
    if (commenteditid && commentreplyid) {
      getCommentReply();
    }
  ...
  }, [commenteditid, commentreplyid, seteditCommentvisible]);
  ...
  const handleComment = async (e) => {
    ...
    try {
      ...
      const result = await axios.put(
        `${backendurl}/api/comments/updatechildcomment/${commenteditid}`,
        { comment, commentreplyid },
        { withCredentials: true }
      );

      toast.success(result.data?.msg || 'Reply updated successfully!');
    ...
    <div className="relative bg-white border border-gray-200 rounded-xl p-3.5 shadow-sm w-full mt-2 transition-all">
      ...
      {loading ? (
        <div className="flex items-center justify-center py-6 text-gray-400 gap-2 text-xs">
          <FaSpinner className="animate-spin text-sm" /> Loading reply...
        </div>
      ) : (
