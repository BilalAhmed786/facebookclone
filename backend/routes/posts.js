...
router.post("/editupload", upload.array("files", 10), async (req, res) => {
  ...
  try {
    const uploadedFiles = await Promise.all(
      req.files.map((file) => uploadToCloudinary(file.buffer, "facebook/posts")),
    );
...
router.delete("/:id", async (req, res) => {
  try {
    ...
    if (post.user.toString() !== req.user.userId) {
      return res.status(403).json("u cannot delete this post");
    }

    // Delete images from Cloudinary
    if (Array.isArray(post.text)) {
      await Promise.all(
        post.text.map((file) => cloudinary.uploader.destroy(file.publicId)),
      );  
    }
...
router.post("/updatepost", async (req, res) => {
  try {
    ...
    if (post.user.toString() !== req.user.userId) {
      return res.status(403).json("u cannot edit this post");
    }

    const oldFiles = Array.isArray(post.text) ? post.text : [];
    const newFiles = Array.isArray(req.body.text) ? req.body.text : [];

    // IDs of images that are still being used
    const newPublicIds = new Set(
      newFiles.filter((file) => file.publicId).map((file) => file.publicId),
    );
    ...
    await Promise.all(
      removedFiles.map((file) => cloudinary.uploader.destroy(file.publicId)),
    );
    ...
    const postdata = await Post.findById(postedit._id)
      .populate("user")
      .populate({
        path: "likes",
        select: "name",
...
router.put("/like/:id", async (req, res) => {
  try {
    ...
    if (!post.likes.includes(req.user.userId)) {
      ...
      const postfind = await Post.findOne({ _id: postsave._id })
        .populate("user")
        .populate({
          path: "likes",
          select: "name",
...
router.get("/timeline/:id", async (req, res) => {
  try {
    ...
    const allPosts = await Post.find({
      $or: [{ user: req.params.id }, { user: req.params.id, isShared: true }],
    })
      .sort({ createdAt: -1 })
      .populate("user")
      .populate({
